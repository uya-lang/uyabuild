...
- `node.workspace` / `node.app` now cover a local Node workspace path with lockfile and workspace-graph scanning: `build` records the root manifests plus reachable workspace package directories, materializes local workspace links, and commits declared frontend outputs; broader npm workspace interop remains follow-up work
