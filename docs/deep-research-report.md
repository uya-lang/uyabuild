...
基于这些事实，我建议的新系统以 **UyaBuild** 为工作名，并先以独立二进制 `uyabuild` 交付。它不是简单替代编译器、包管理器、Docker 或 Bazel，而是做统一的**构建控制面**：用一种**无 tab、单赋值操作符、显式默认目标、任务与产物分离、插件化规则包、内容哈希驱动增量、沙箱/追踪校验隐藏依赖、BEP 风格事件协议**的新模型，来编排 C/C++、JS 前端、Docker 镜像、多语言 monorepo 乃至微服务交付流水线。CMake、Ninja、Docker Buildx、npm Workspaces、Bazel Query/BEP/缓存等已有能力，不应被推倒重来，而应通过导入、导出、适配和后端复用纳入统一体系。这里的关键收束是两点：**实现语言优先选 Uya 本身**，以及**用户入口收敛到 `uyabuild build / test / query / why`**，而不是继续维护一套平行的外部构建工具。
...
| 默认入口 | **把 `uyabuild build` 作为主入口** | 用户应记住的是 `uyabuild`，而不是 `make` 或另一套脚本入口；`uyabuild test/query/why/release` 也可自然并列 |
| 自举边界 | **保留一个极小的 bootstrap 层** | Phase 0 需要一个外部已有 `uya` 编译器来编译纯 `uya` 源码，避免设计再引入新的宿主语言依赖 |
| 迁移策略 | **先让 Makefile 退化为兼容壳层** | 第一阶段不追求一次性删掉 Make，而是把它收缩成 `bootstrap` 和命令转发层，逐步退场 |

更具体地说，我建议把 `uya.build` 提升为全生态统一入口：根 `uya.build` 同时承载 workspace 元数据、目标图与规则声明，`uyabuild` 二进制负责解析、规划、执行和解释构建结果；现有 `uya.toml` 则降级为可选兼容清单，仅在迁移期或外部工具集成时保留静态元数据。这样 `uya` 语言和 `uyabuild build` 构建前端会形成更直接的自举闭环，同时又不给现有生态制造硬切换成本。
...
我建议迁移按“五步法”推进，而不是直接重写。对 `uya` 本仓来说，还应额外补一个**第零阶段 bootstrap 约束**：先允许一个极小的 `make bootstrap` 或 `./bootstrap.sh` 只负责从纯 `uya` 源码得到首个可运行 `bin/uyabuild`，一旦这个边界跨过去，后续开发者入口就统一切到 `uyabuild build/test/check/release/install`。
...
| `make uya` / `make uya-hosted` / `make uya-portable` | `uyabuild build //compiler:uya` | hosted/nostdlib/portable 变成配置矩阵，而不是不同手写 recipe |
| `make tests` / `make tests-hosted` | `uyabuild test //tests/...` | 测试选择与 profile 进入显式目标图 |
| `make check` / `make check-hosted` | `uyabuild check //...` | 汇总验证逻辑进入结构化 pipeline，而不是大段 shell 串联 |
| `make release` / `make release-clean` | `uyabuild release //dist:compiler` | 干净树、seed 同步、产物归档变成可解释的阶段节点 |
| `make install` | `uyabuild install //dist:compiler` | 安装布局由 manifest 和目标输出契约驱动 |
...
| 组件 | 职责 | 原型实现建议 |
|---|---|---|
| Bootstrap shim | 从纯 `uya` 源码得到首个可运行 `bin/uyabuild`，然后转交给 UyaBuild | 保留极小 `make bootstrap` / `bootstrap.sh`，避免自举循环 |
...
uyabuild query 'revdeps(//..., //lib:core)'
...
| 里程碑 | 必须交付 | 不该强求 |
|---|---|---|
| bootstrap 收口 | `make bootstrap` 或等价脚本只负责产出首个 `bin/uyabuild`，之后默认入口全部切到 `uyabuild ...` | 不急于第一天就彻底删除 Make |
...
| 可迁移 | 能稳定包装 Make/CMake/npm/Docker 子系统，并让 `uya` 仓库主流程默认走 `uyabuild build/test/check/release` | 不急于完全自动翻译所有语法 |
...
| Uya 默认化 | `uya` 仓库开发文档与 CI 主路径默认使用 `uyabuild build/test/check/release`，顶层 Makefile 只保留 bootstrap 和兼容转发 |
